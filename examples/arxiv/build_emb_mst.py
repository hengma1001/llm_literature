...
embedding_model = LlamaCppSettings(
    model_path="/homes/heng.ma/Research/md_pkgs/llm-scripts/langchain/mistral-openorca/mistral-7b-openorca.Q4_K_M.gguf",
...
for i, row in tqdm(df.iterrows(), total=len(df), desc="Embedding Prog:"):
    # print(row.abstract)
    emb = embedding_model.embed_query(row.abstract.split("."))
...
emb_df.to_pickle("arxiv_msd.pkl")
